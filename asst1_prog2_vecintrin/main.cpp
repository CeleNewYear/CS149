...
void clampedExpVector(float* values, int* exponents, float* output, int N)
{
    ...
    for (; i < N - VECTOR_WIDTH; i += VECTOR_WIDTH) {
        ...
        // Load vector of values from comtinuous memory addresses
        _cs149_vload_float(x, values + i, maskAll);

        // Load vector of exponents from comtinuous memory addresses
        _cs149_vload_int(y, exponents + i, maskAll);

        // Set mask according to predicte
        _cs149_vlt_int(maskIsZero, y, one, maskAll); // if (y < 0) {
        ...
        // Execute instruction ("else" clause)
        _cs149_vmove_float(result, x, maskIsNotZero); //   output[i] = x; }

        // y = y - 1
        _cs149_vsub_int(y, y, one, maskIsNotZero);

        // Set mask according to predicate
        _cs149_vlt_int(maskIsNotZero, zero, y, maskIsNotZero); // if (y > 0) {
        ...
        // predicate = result > max
        _cs149_vgt_float(maskIsNotZero, result, max, maskAll);

        // Execute instruction using mask ("if" clause)
        _cs149_vmove_float(result, max, maskIsNotZero); //   output[i] = 9.999999f;

        maskIsZero = _cs149_mask_not(maskIsNotZero); // } else {

        // Execute instruction ("else" clause)
        _cs149_vmove_float(result, result, maskIsZero); //   output[i] = result; }
...
float arraySumVector(float* values, int N)
{
    ...
    res = arraySumSerial((float*)&sum, VECTOR_WIDTH);
