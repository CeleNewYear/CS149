/**
 * brief: 2024-12-4没有完全搞定，调用ispc中调用launch[2]会编译失败 "mandelbrot_ispc_withtasks"函数
 * undefined reference to `ISPCAlloc'
 * undefined reference to `ISPCLaunch'
 * undefined reference to `ISPCSync'
 *
 * 如果CMakeLists.txt中添加"E:/2025codestudy/CS241Parallel/ispc-v1.25.3-windows/lib/ispcrt.lib"，则程序可以编译通过，但无法运行
 */
